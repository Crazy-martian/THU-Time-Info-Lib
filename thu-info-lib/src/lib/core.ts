...
const parsePortalPayload = <T = any>(payload: string): T => {
    try {
        return JSON.parse(payload) as T;
    } catch (error) {
        const normalized = payload.trim().replace(/^\(/, "").replace(/\);?$/, "");
        console.log("[thu-info debug] parsePortalPayload: JSON.parse failed, preview =", payload.slice(0, 200));
        try {
            return Function(`"use strict"; return (${normalized});`)() as T;
        } catch (fallbackError) {
            console.log("[thu-info debug] parsePortalPayload: fallback parse failed, normalized preview =", normalized.slice(0, 200));
            console.log("[thu-info debug] parsePortalPayload: original error =", error);
            console.log("[thu-info debug] parsePortalPayload: fallback error =", fallbackError);
            throw fallbackError;
        }
    }
};
...
export const roam = async (helper: InfoHelper, policy: RoamingPolicy, payload: string): Promise<string> => {
    ...
    case "default": {
        ...
        const {object} = parsePortalPayload<{object: {roamingurl: string}}>(
            await uFetch(`${ROAMING_URL}?yyfwid=${payload}&_csrf=${csrf}&machine=p`),
        );
...
export const verifyAndReLogin = async (helper: InfoHelper): Promise<boolean> => {
    ...
    try {
        const {object} = parsePortalPayload<{object: {ryh: string}}>(
            await uFetch(`${USER_DATA_URL}?_csrf=${await getCsrfToken()}`),
        );
