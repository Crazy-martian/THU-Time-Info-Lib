...
const parseLooseJson = <T = any>(payload: string): T => {
    try {
        return JSON.parse(payload) as T;
    } catch (error) {
        const normalized = payload.trim().replace(/^\(/, "").replace(/\);?$/, "");
        console.log("[thu-info debug] parseLooseJson: JSON.parse failed, preview =", payload.slice(0, 200));
        try {
            return Function(`"use strict"; return (${normalized});`)() as T;
        } catch (fallbackError) {
            console.log("[thu-info debug] parseLooseJson: fallback parse failed, normalized preview =", normalized.slice(0, 200));
            console.log("[thu-info debug] parseLooseJson: original error =", error);
            console.log("[thu-info debug] parseLooseJson: fallback error =", fallbackError);
            throw fallbackError;
        }
    }
};
...
export const getPhysicalExamResult = (
    ...
    roamingWrapperWithMocks(
        ...
        () => uFetch(PHYSICAL_EXAM_URL).then((s) => {
            const json = parseLooseJson<any>(s);
